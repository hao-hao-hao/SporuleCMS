...
class Post(db.Model, DB_Base):
    ...
    def get_all_items_pagination(page=1, per_page=10, error_out=True):
        posts = Post.query.filter(Post.post_date <= datetime.now()).order_by(db.desc(Post.post_date)).paginate(
            page, per_page, error_out)
        return posts

    @staticmethod
    def get_all_items_pagination_admin(page=1,per_page=30,error_out=True):
    ...
    def generate_excerpt(self, length):
        ...
        excerpt = tagcloser.close_tags(excerpt)
        return excerpt
